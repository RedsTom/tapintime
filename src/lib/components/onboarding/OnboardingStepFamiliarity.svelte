...
<div class="flex flex-col gap-6 text-left">
	<div class="flex items-center justify-between">
		...
		<span class="text-xs font-black uppercase text-text-dim">Niveau d'accoutumance</span>
	</div>

	<div class="flex flex-col gap-2 mt-2">
		<label class="text-sm md:text-base font-black uppercase tracking-wider text-text flex items-center gap-2">
			<Eye class="w-5 h-5 text-primary" /> Quel est votre niveau d'aisance ?
		</label>
		<span class="text-xs font-black uppercase text-text-dim/70 leading-tight">
			Sélectionnez le niveau avec lequel vous êtes à l'aise pour pré-débloquer les touches correspondantes.
		</span>
	...
	{#if previewLayout}
		<div class="bg-surface border-4 border-secondary p-4 rounded-xl shadow-neo flex flex-col gap-4 text-center mt-2 relative overflow-hidden">
			<h3 class="text-xs font-black uppercase tracking-wider text-primary">Touches débloquées (Aperçu)</h3>
			<div class="w-full flex justify-center py-2 max-w-full overflow-x-auto scrollbar-none">
				<VirtualKeyboard 
					layout={previewLayout} 
					pressedKeys={new Set()} 
					unlockedKeys={new Set(getUnlockedKeys(0, layoutFamiliarity))}
					scale={0.7} 
				/>
			</div>
		</div>
